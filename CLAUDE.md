...
- **`/mcp`** is served by `SwapMcpAgent.serve("/mcp", { binding: "SwapMcpAgent" })`, fronted by `transportGuard` before the transport is reached. The guard validates `Origin` against `ALLOWED_ORIGINS` **only when the header is present** (a browser-only DNS-rebinding defense); server-side MCP connectors (Claude/GPT/Grok) send no `Origin` and are authenticated by the bearer token. It does **not** require `MCP-Protocol-Version` — the transport negotiates the version (spec: absent ⇒ default `2025-03-26`).
