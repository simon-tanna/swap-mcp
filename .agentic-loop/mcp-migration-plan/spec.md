...
**Repo:** `swap-mcp` · **Branch:** `feat/mcp-migration-plan` · **Date:** 2026-07-12 · **Version:** v3.2
...
**Changelog — v3.2 (validator-prescribed mechanical amendments to v3.1):** (a) §5.8 executeSwap steps reordered so the balance/gas-headroom rail runs *after* `buildSwap` (the rail needs `SwapTx.gasLimit`, still before sign/submit; abort → `pending→failed`, no tx sent); (b) §5.8 M3 receipt error-branching gains an explicit default — any non-timeout throw (RPC error, replacement, receipt-not-found) leaves the row `submitted` with `txHash`, call result `timed_out`, never `failed`; (c) §5.8 concurrency note records that the in-DO promise-chain mutex is per-live-instance (does not survive DO eviction/hibernation), with cross-eviction safety resting on the single-in-flight-swap invariant + D1 reconciliation.
...
- **Responsibility:** Serialize **all** swap execution for hot-wallet nonce safety (one in-flight swap at a time per the single wallet) via an **in-DO promise-chain single-flight mutex** — each `executeSwap` call appends to a serialized promise chain so submissions never overlap. **The serialization primitive is an in-DO promise-chain mutex, NOT `blockConcurrencyWhile` per request** (which is for init/consistency windows, not application-level queuing). For each swap, the lifecycle is:
  ...
  5. Enforce the input-shape safety rails that need no gas figure (amount > 0; slippage ≤ 5%; deadline bound). Any rail failure here **writes the `pending` row to `failed`** with the corresponding `errorCode` and sends no transaction (§ M6).
  6. Obtain Universal Router calldata via `/swap` (the API embeds the slippage-derived minimum-output floor inside the returned calldata). The returned `SwapTx` carries the `gasLimit` the next step needs.
  7. **Balance / gas-headroom rail (runs AFTER `buildSwap` — it needs `SwapTx.gasLimit`):** enforce `amount ≤ balance` with the direction-aware balance source and gas headroom (§5.9 / M1/M2) using `gasLimit × maxFeePerGas`. Still **before sign/submit** — a shortfall **aborts**: write the `pending` row to `failed` with `errorCode = insufficient_balance` and send **no transaction** (§ M6).
  8. Sign + submit via viem → row `submitted` + `txHash`.
  9. `waitForTransactionReceipt` → row `confirmed` (with `actualAmountOut`, `gasUsed`) or `failed` (a reverted swap's `failed` row retains its `txHash`, unlike pre-submit aborts).
- **Receipt-wait error disambiguation (M3):** `waitForTransactionReceipt` throws on **both** timeout and revert; the coordinator **branches on the error type**. A `WaitForTransactionReceiptTimeoutError` (or the configured bound elapsing) → **row stays `submitted`**, call result `timed_out` (never mapped to `failed`). A **revert** (receipt with `status:"reverted"`, or a revert error) → **row `failed`** with `errorCode = swap_failed`. A timeout is **never** written as `failed`. **Explicit default branch (any non-timeout throw):** every throw that is *not* a genuine `status:"reverted"` receipt — RPC error, transaction replacement, receipt-not-found, or any other non-timeout error — is treated exactly like a timeout: the **row stays `submitted`** with its `txHash`, call result `timed_out` (reconcile path), **never `failed`**. `failed`/`swap_failed` is reserved for a genuine `status:"reverted"` receipt; only that outcome writes `failed`.
...
- **Concurrency note (mutex scope):** the in-DO promise-chain mutex is **per-live-instance** — it lives in memory on the running DO instance and **does not survive DO eviction/hibernation**, so it cannot serialize across an eviction boundary; cross-eviction safety instead rests on the single-in-flight-swap invariant (one wallet, one swap at a time) plus D1 reconciliation of any `submitted`-stranded row.
